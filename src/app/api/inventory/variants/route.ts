...
export async function GET() {
  ...
  const db = getDb();
  const rows = await db
    .select({ variantId: productVariants.variantId, modelName: productVariants.modelName, modelGroup: productVariants.modelGroup, color: productVariants.color, sellingPriceNtd: productVariants.sellingPriceNtd })
